<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Application Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used throughout the app for various
    | purposes.
    |
    */

    'bad_request'                   => 'Hoppla! Da ist etwas schiefgelaufen.',
    'by'                            => 'von',
    'close'                         => 'Schließen',
    'comments'                      => 'Kommentare',
    'copy_to_clipboard'             => 'In die Zwischenablage kopieren',
    'dashboard'                     => 'Dashboard',
    'delete'                        => 'löschen',
    'delete_comment'                => 'Kommentar löschen',
    'edit'                          => 'bearbeiten',
    'edit_comment'                  => 'Kommentar bearbeiten',
    'flag'                          => 'melden',
    'last_edit'                     => 'zuletzt bearbeitet',
    'manual_link'                   => 'Hier klicken, um Hilfe zu diesem Thema zu erhalten',
    'no_comments'                   => 'Bisher gibt es keine Kommentare.',
    'post_comment'                  => 'Senden',
    'presented_with'                => 'Präsentiert mit',
    'really_delete_comment_body'    => 'Möchtest du diesen Kommentar wirklich löschen?',
    'really_delete_comment_title'   => 'Kommentar löschen',
    'toggle_navigation'             => 'Navigation ein-/ausblenden',
    'write_a_comment'               => 'Verfasse einen Kommentar...',
    'your_comment'                  => 'Neuen Kommentar hinzufügen',
];
